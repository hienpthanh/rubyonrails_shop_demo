class ProductsController < ApplicationController
  ...
  def index
    @products = Product.get_all().paginate(:page => params[:page], :per_page => PAGE_CONST)
    @page_title = 'Sản Phẩm'
  end
  ...
  # GET /products/1.json
  def show
    @products = Product.get_show(params[:id])
  end
  # GET /products/new
  def new
    @product = Product.new
  ...
  def create
    ...
    respond_to do |format|
      if Product.add(product_params, @product)
        format.html { redirect_to products_url, notice: 'Thêm Sản Phẩm Thành Công' }
