module ApplicationHelper
	...
	def sidebar_category
		@categories = Category.get_all
		render template: "layouts/sidebar_category"
	end
	def slider_product
		@slider_product  = Product.get_products('slider',10)
		render template: "index/slider_product"
	end	
    ...
    def generate_new_url(params)
        ...
        if(explode_url.length > 1 && end_explode_url.size > 0)
            return url.to_s + '&' + query_string.to_s;
        else
            url = url.gsub!('?', '')
            return url.to_s + '?' + query_string.to_s;
        end   
	...
	def pagination(config)
		...
		@limit_per_page = config[:limit_per_page] ? Float(config[:limit_per_page]) : 50
		...
		distance    = page_range.even ? ((Float(page_range) - 1) / 2).floor : (Float(page_range) / 2).floor
